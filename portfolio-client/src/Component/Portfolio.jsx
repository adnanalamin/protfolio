import project1 from '../assets/project 1.png'
const Portfolio = () => {
  return (
    <div id='portfolio' className="bg-[#31065A] pb-32">
      <div className="w-full text-center pt-16 pb-16">
        <h2 className="font-ex2 font-semibold text-white text-[40px]">
          My <span className="text-[#F86F03]">Portfolio</span>
        </h2>
        <hr className="w-64 bg-[#D9D9D9] mx-auto" />
      </div>
      <div className="grid grid-cols-3 gap-2 max-w-6xl mx-auto">
        <div className="mb-4">
          <div className="relative rounded-xl max-w-sm overflow-hidden bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52  rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <div className="relative max-w-sm overflow-hidden rounded-xl bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52 rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <div className="relative max-w-sm overflow-hidden rounded-xl bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52 rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <div className="relative max-w-sm rounded-xl overflow-hidden bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52 rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <div className="relative max-w-sm overflow-hidden rounded-xl bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52 rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
        <div className="mb-4">
          <div className="relative max-w-sm overflow-hidden rounded-xl bg-cover bg-no-repeat">
            <img
              src={project1}
              className="max-w-sm h-52 rounded-xl"
              alt="Louvre"
            />
            <div className="absolute bottom-0 rounded-xl left-0 right-0 top-0 h-full w-full overflow-hidden bg-[#AD5BFBCC] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100">
              <div className="flex flex-col items-center pt-20">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="25"
                  height="25"
                  viewBox="0 0 25 25"
                  fill="none"
                >
                  <path
                    d="M21.0938 15.625H19.5312C19.324 15.625 19.1253 15.7073 18.9788 15.8538C18.8323 16.0003 18.75 16.199 18.75 16.4062V21.875H3.125V6.25H10.1562C10.3635 6.25 10.5622 6.16769 10.7087 6.02118C10.8552 5.87466 10.9375 5.67595 10.9375 5.46875V3.90625C10.9375 3.69905 10.8552 3.50034 10.7087 3.35382C10.5622 3.20731 10.3635 3.125 10.1562 3.125H2.34375C1.72215 3.125 1.12601 3.37193 0.686468 3.81147C0.24693 4.25101 0 4.84715 0 5.46875L0 22.6562C0 23.2779 0.24693 23.874 0.686468 24.3135C1.12601 24.7531 1.72215 25 2.34375 25H19.5312C20.1529 25 20.749 24.7531 21.1885 24.3135C21.6281 23.874 21.875 23.2779 21.875 22.6562V16.4062C21.875 16.199 21.7927 16.0003 21.6462 15.8538C21.4997 15.7073 21.301 15.625 21.0938 15.625ZM23.8281 0H17.5781C16.5347 0 16.0132 1.26514 16.748 2.00195L18.4927 3.74658L6.5918 15.6431C6.48255 15.7519 6.39586 15.8813 6.33672 16.0237C6.27757 16.1662 6.24712 16.3189 6.24712 16.4731C6.24712 16.6274 6.27757 16.7801 6.33672 16.9225C6.39586 17.065 6.48255 17.1944 6.5918 17.3032L7.69873 18.4082C7.8076 18.5175 7.93697 18.6041 8.07941 18.6633C8.22186 18.7224 8.37457 18.7529 8.52881 18.7529C8.68304 18.7529 8.83576 18.7224 8.9782 18.6633C9.12065 18.6041 9.25002 18.5175 9.35889 18.4082L21.2539 6.50977L22.998 8.25195C23.7305 8.98438 25 8.47168 25 7.42188V1.17188C25 0.861074 24.8765 0.563003 24.6568 0.343234C24.437 0.123465 24.1389 0 23.8281 0Z"
                    fill="white"
                  />
                </svg>
                <a className="font-ex2 text-xl text-white font-semibold mt-2">Web Design</a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Portfolio;
